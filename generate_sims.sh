...
SIMS=(
  "OrbitalSim" "CollisionSim" "FluidSim" "ThermoSim" "OpticsSim" "EMSim" "StaticsSim"
  "MolecularBuilderSim" "TitrationSim" "GasLawSim" "KineticsSim" "LatticeSim"
  "OsmosisSim" "NeuronSim" "DNASim" "EcosystemSim" "EpidemiologySim"
)

for SIM in "${SIMS[@]}"; do
  cat << INNER_EOF > "src/simulations/${SIM}.jsx"
import React, { useEffect, useRef } from 'react';
import * as BABYLON from '@babylonjs/core';
import { createLabEnvironment, createLabLighting, createLabCamera, enableWebXR } from '../utils/labEnvironment';

const ${SIM} = ({ settings, isRunning, triggerReset }) => {
...
echo "17 simulation MVPs created successfully."
